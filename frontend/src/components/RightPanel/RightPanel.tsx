...
function EffectsIcon({ active }: { active: boolean }) {
  return (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke={active ? "#7c3aed" : "var(--txt3)"} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
...
function MediaIcon({ active }: { active: boolean }) {
  return (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke={active ? "var(--accent)" : "var(--txt3)"} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
      <rect x="2" y="4" width="16" height="13" rx="2" />
      <path d="M8 8.5l5 2.5-5 2.5V8.5z" fill={active ? "var(--accent)" : "var(--txt3)"} stroke="none" />
    </svg>
  );
}
...
export default function RightPanel() {
  ...
  return (
    <div className="flex flex-shrink-0">
      ...
      {rightPanelTab !== null && (
        <div className="w-[260px] flex flex-col bg-[var(--panel)] border-l border-r border-[var(--border)]">
          <div className="flex items-center gap-2 px-3 py-2 border-b border-[var(--border)] flex-shrink-0">
            <button
              type="button"
              onClick={() => setRightPanelTab(null)}
              className="p-1 rounded hover:bg-[var(--hover)] text-[var(--txt2)] hover:text-[var(--txt1)] transition-colors cursor-pointer"
              title="Close panel"
            >
            ...
            <span className="text-[11px] text-[var(--txt2)] font-normal">
          ...
          <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
            {rightPanelTab === "properties" ? (
              selectedItemIds.size > 1 ? (
                <div className="flex flex-col items-center justify-center flex-1 gap-2 text-[var(--txt2)] p-6">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    <rect x="3" y="3" width="7" height="7" rx="1" /><rect x="14" y="3" width="7" height="7" rx="1" />
                    <rect x="3" y="14" width="7" height="7" rx="1" /><rect x="14" y="14" width="7" height="7" rx="1" />
                  </svg>
                  <span className="text-[13px] font-medium text-[var(--txt2)]">{selectedItemIds.size} items selected</span>
                  <span className="text-[11px] text-center text-[var(--txt2)]">Drag any selected item to move them all</span>
                </div>
      ...
      {/* Vertical icon strip */}
      <div className="w-[64px] flex flex-col items-center pt-4 gap-1 bg-[var(--panel-2)] border-l border-[var(--border)]">
        {TABS.map(({ id, label, icon }) => {
          ...
          return (
            ...
            >
              {icon(active)}
              <span className={`text-[11px] font-normal leading-none ${active ? "text-[var(--accent)]" : "text-[var(--txt2)]"}`}>
                {label}
              </span>
            </button>
          );
