...
export default function WaveformCanvas({ fileId, fileDuration, sourceStart, sourceEnd, width, height, volume = 1, opacity = 0.65 }: Props) {
  ...
  useEffect(() => {
    ...
    ctx.fillStyle = `oklch(70% 0.16 55 / ${opacity})`;
    for (let x = 0; x < W; x++) {
      const sIdx = startIdx + Math.floor((x / W) * visibleSamples);
      const amp = waveform[Math.min(sIdx, totalSamples - 1)] ?? 0;
      const barH = Math.min(CH, amp * Math.max(0.15, volume) * CH);
      ctx.fillRect(x, (CH - barH) / 2, 1, Math.max(1, barH));
    }
  }, [waveform, sourceStart, sourceEnd, fileDuration, width, height, volume, opacity]);
