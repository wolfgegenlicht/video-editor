...
def trans_module(inputs, structures, phase_train, name="trans_module"):
    with tf.variable_scope(name) as scope:
        ...
        cnn_4 = tf_layers.conv2d(inputs=cnn_blk_3, filters=structures['depth'][4], kernel_size=structures['filter_size'][4], padding="same", activation=None, use_bias=False, name="cnn_4")
        return cnn_4
...
def lcm_module(inputs, structures, phase_train, name="lcm_module"):
    with tf.variable_scope(name) as scope:
        cnn_blk_0 = tf_utils.cnn_blk(inputs, structures['depth'][0], structures['filter_size'][0], phase_train, name = 'cnn_blk_0')        
        cnn_blk_1 = tf_utils.cnn_blk(cnn_blk_0, structures['depth'][1], structures['filter_size'][1], phase_train, name = 'cnn_blk_1')
        cnn_2 = tf_layers.conv2d(inputs=cnn_blk_1, filters=structures['depth'][2], kernel_size=structures['filter_size'][2], padding="same", activation=None, use_bias=False, name='cnn_2')
...
def inference(input_img, input_fp, input_agl, phase_train, conf):
    ...
    with tf.variable_scope('warping_model'):        
        ...
        with tf.variable_scope('warping_module'):
            '''coarse module'''
            resized_igt_inputs = tf_layers.average_pooling2d(inputs=igt_inputs, pool_size=[2,2], strides=2, padding='same')
            ...
            coarse_out = tf_layers.average_pooling2d(inputs=coarse_resize, pool_size=[2, 2], strides=1, padding='same')
            '''fine module'''
