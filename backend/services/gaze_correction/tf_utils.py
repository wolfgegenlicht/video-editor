...
def batch_norm(x, train_phase, name='bn_layer'):
    ...
    batch_norm = tf_layers.batch_normalization(
            inputs=x,
...
def cnn_blk(inputs, filters, kernel_size, phase_train, name = 'cnn_blk'):
    with tf.variable_scope(name) as scope:
        cnn = tf_layers.conv2d(inputs=inputs, filters=filters, kernel_size=kernel_size, padding="same", activation=None, use_bias=False, name="cnn")
...
def dnn_blk(inputs, nodes, name = 'dnn_blk'):
    with tf.variable_scope(name) as scope:
        dnn = tf_layers.dense(inputs=inputs, units=nodes, activation=None, name="dnn")
